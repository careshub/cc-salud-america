...
function sa_leader_report() {
    ...
    <div class="content-row clear">
        <div id="sa-report-selection">
            <div id="select-county">Select a state and county to generate your leader report:</div>
            <select id="state-list">
                <option>--- Select a State ---</option>
                <?php
                    // get a list of states - exclude territories since we don't have data to generate report for
                    $json_states = sa_leader_report_get_api('api-location/v1/geoid-list?geo_key=040');
            ...
            <span id="report-wait-message">Generating report, please wait...</span>
        </div>

        <?php
        // if we have valid county geoid, generat the report.
        if ( $geoid && strlen($geoid) === 12 ) :
            $group_url = bp_get_group_permalink();
            $image_url = plugins_url('images/leader-report/', dirname(__FILE__));

			// get county FIPS code
			$fips = preg_replace("/[^',]*US/", '', $geoid);
			
            // get data for cover page
            $cover_page = sa_leader_report_cover_page($fips);
            ...
            $obesity_page = sa_leader_report_obesity_page($fips);

            // get data for fast food page
            $fast_food_page = sa_leader_report_food_access_page($fips);

            // get data for health equity page
            $health_equity_page = sa_leader_report_health_equity_page($fips);

            // get data for vulnerable population page
            $vulnerable_population_page = sa_leader_report_vulnerable_population_page($geoid);
         ...
         <div id="sa-report-content">
             <div id="cover-page">
                ...
                <div class="center-align sa-report-changes">
                    <img src="<?php echo $image_url?>sa-active-spaces.png" />
                    <img src="<?php echo $image_url?>sa-healthier-schools.png" />
                    <img src="<?php echo $image_url?>sa-better-food.png" />
                    <img src="<?php echo $image_url?>sa-healthy-weight.png" />
                    <img src="<?php echo $image_url?>sa-sugary-drinks.png" />
                    <img src="<?php echo $image_url?>sa-health-equity.png" />
                </div>
                ...
                <div class="sa-report-spacing2"></div>
                 ...
                 <div class="w3-row">
                       <div class="w3-third sa-col-end sa-col-media center-align">
                        ...
                        <p class="sa-report-font-2">Salud Heroes: How to Start a Baby Caf&#233;</p>
...
function sa_leader_report_get_api($api){
...
function sa_leader_report_get_json($fips, $id='', $param = '', $api_name ='indicator'){
    $api = 'api-report/v1/';
    $api .= $api_name . '/Salud/' . $id . '?area_type=county&area_ids=' . $fips . $param;
    return sa_leader_report_get_api($api);
}

// get indicator dial
function sa_leader_report_get_dial($json){
    return "<p><b>". $json->data->gauge_label . "</b></p>" . $json->gauge;
}

// get indicator value as double
function sa_leader_report_get_double($value){
...
function sa_leader_report_get_single_digit_pct($value){
    if ($value == 0) {
        return "0%";
    }else{
...
// get fraction expression for latino adults & kids, called from sa_leader_report_cover_page()
function sa_leader_report_set_fraction_value($value){
    if ($value > 0){
...
function sa_leader_report_cover_page($fips){
    $data = (object) array(
                'county_name' => '',
                'state_name' =>'',
                ...
                'pct_latino_kids' =>0,
                'health_ranking' =>'',
                'health_ranking_total' => '',
                'health_ranking_suffix' => ''
            );

    try{
        // Latino adults - percentage and fraction
        $json_value = sa_leader_report_get_json($fips, '7080', '&breakout_id=8269');        // age 18+
        ...
        $value_double = sa_leader_report_get_double($json_value->data->summary->values[3]);
        $data->pct_latino_adults = sa_leader_report_get_single_digit_pct($value_double);
        $data->frac_latino_adults =    sa_leader_report_set_fraction_value($value_double);

        // Latino kids - percentage and fraction
        $json_value = sa_leader_report_get_json($fips, '706', '&breakout_id=8590');
        $value_double = sa_leader_report_get_double($json_value->data->summary->values[3]);
        $data->pct_latino_kids = sa_leader_report_get_single_digit_pct($value_double);
        $data->frac_latino_kids =    sa_leader_report_set_fraction_value($value_double);

        // health outcoome ranking
        $json_value = sa_leader_report_get_json($fips, '8040');
...
function sa_leader_report_obesity_page($fips){
    $data = (object) array(
        'pct_overweight'=>-1
        );
    try{
        // state overall percentage overweight or obese
        $json_value = sa_leader_report_get_json($fips, '8018', '&breakout_id=3');
        $value_double = sa_leader_report_get_double($json_value->data->state_list[0]->values[1]);
        $data->pct_latino_obese = sa_leader_report_get_single_digit_pct($value_double);
        $value_double = sa_leader_report_get_double($json_value->data->state_list[0]->values[2]);
        $data->pct_white_obese = sa_leader_report_get_single_digit_pct($value_double);

        // obese percentage and dial - should always have data for counties except PR
        $json_value = sa_leader_report_get_json($fips, '603', "&output_gauge=true");
        console_log($json_value);
        $data->dial_obese = sa_leader_report_get_dial($json_value);
        $value_double = sa_leader_report_get_double($json_value->data->summary->values[3]);
        $data->pct_obese = sa_leader_report_get_single_digit_pct($value_double);

        // overweight percentage and dial - may not have data for the county
        $json_value = sa_leader_report_get_json($fips, '604', "&output_gauge=true");
        $data->dial_overweight = sa_leader_report_get_dial($json_value);
        $value_double = sa_leader_report_get_double($json_value->data->summary->values[3]);
        if ($value_double !== 0){
            $data->pct_overweight = sa_leader_report_get_single_digit_pct($value_double);
        }
    }catch(Exception $e){
        console_log($e->getMessage());
    }
...
function sa_leader_report_food_access_page($fips){
    $data = (object) array(
        'total_fruit'=>-1
        );
    try{
        // fast food dial
        $json_value = sa_leader_report_get_json($fips, '401', "&output_gauge=true");
        $data->dial_fast_food = sa_leader_report_get_dial($json_value);

        // grovery store
        $json_value = sa_leader_report_get_json($fips, '402', "&output_gauge=true");
        $data->dial_grocery = sa_leader_report_get_dial($json_value);

        // fruit/veggis - may not have value
        $json_value = sa_leader_report_get_json($fips, '301', "&output_gauge=true");
        $data->dial_fruit = sa_leader_report_get_dial($json_value);

        $value_double = sa_leader_report_get_double($json_value->data->summary->values[3]);
        if ($value_double !== 0){
            $data->total_fruit = $json_value->data->summary->values[2];
            $data->pct_fruit = sa_leader_report_get_single_digit_pct($value_double);
        }
    }catch(Exception $e){
        console_log($e->getMessage());
    }
...
function sa_leader_report_physical_activity_page($fips){
    $data = (object) array();
    try{
         // recreation
        $json_value = sa_leader_report_get_json($fips, '408', "&output_gauge=true");
        $data->dial_recreation = sa_leader_report_get_dial($json_value);

        // physical inactivity
        $json_value = sa_leader_report_get_json($fips, '306', "&output_gauge=true");
        $data->dial_physical = sa_leader_report_get_dial($json_value);
        $data->total_physical = $json_value->data->summary->values[2];
        $value_double = sa_leader_report_get_double($json_value->data->summary->values[3]);
        $data->pct_physical = sa_leader_report_get_single_digit_pct($value_double);

        // pedestrian
        $json_value = sa_leader_report_get_json($fips, '627', "&output_gauge=true");
        $data->dial_pedestrian = sa_leader_report_get_dial($json_value);
    }catch(Exception $e){
        console_log($e->getMessage());
    }
    return $data;
...
function sa_leader_report_health_equity_page($fips){
    $data = (object) array();
    try{
        // children in poverty
        $json_value = sa_leader_report_get_json($fips, '781', "&output_gauge=true");
        $data->dial_poverty = sa_leader_report_get_dial($json_value);
        $data->total_poverty = $json_value->data->summary->values[3];
        $value_double = sa_leader_report_get_double($json_value->data->summary->values[4]);
        $data->pct_poverty = sa_leader_report_get_single_digit_pct($value_double);

        // access to primary care
        $json_value = sa_leader_report_get_json($fips, '505', "&output_gauge=true");
        $data->dial_primary_care = sa_leader_report_get_dial($json_value);

        // no insurance
        $json_value = sa_leader_report_get_json($fips, '202', "&output_gauge=true");
        $data->dial_insurance = sa_leader_report_get_dial($json_value);

        // socio-economic - high school graduation
        $json_value = sa_leader_report_get_json($fips, '760', "&breakout_id=465");
        ...
        $json_value = sa_leader_report_get_json($fips, '781', "&breakout_id=925");
        ...
        $json_value = sa_leader_report_get_json($fips, '202', "&breakout_id=917");
...
function sa_leader_report_vulnerable_population_page($geoid){
    $data = (object) array(
        'pct_white'=>-1
        );
    try{
        // get data for vulnerable population
        $json_value = sa_leader_report_get_json('', $geoid, '&map_width=600&map_height=610', 'vpf');

        if ($json_value->values != null){
            $data->pct_white = sa_leader_report_get_single_digit_pct($json_value->values[0]->value);
            $data->pct_latino = sa_leader_report_get_single_digit_pct($json_value->values[1]->value);
